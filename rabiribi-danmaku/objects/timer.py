...
class Number(pygame.sprite.Sprite):
    ...
    def load_sources(cls, number_type):
        ...
        cls.image = pygame.image.load(file_name).convert()
        cls.rect = cls.image.get_rect() # force square per character
    
    def resize(self):
        self.image = pygame.transform.scale(self.image, (self.rect.width*self.size/self.rect.heithg, self.size))
    ...
    def __setattr__(self, name, value):
        if name == 'number':
            ...
            if L != self.number_size:
                ...
                super().__setattr__('surface', pygame.surface.Surface((value*self.size, self.size)))
